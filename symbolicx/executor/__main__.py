...
from symbolicx.common.exceptions import GenericException
from .display import (
    display,
    start_display,
    stop_display,
...
def run_symbolicx(arguments: List[str]) -> None:
    ...
    while args.max_iters is None or iter_cnt < args.max_iters:
        ...
        new_echidna_inputs_cnt = len(
            [
                f
                for f in new_inputs
                if not os.path.basename(f).startswith("symbolicx")
            ]
        )
...
def run_symbolicx_with_display(args: List[str]) -> None:
...
def main() -> None:
    func = (
        ...
        else run_symbolicx_with_display
    )
    func(sys.argv[1:])
